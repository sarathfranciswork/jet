...
class SignInEndpointTests(BaseAPITest):
    def setUp(self):
        super().setUp()
        user = User.objects.create(email="user@jetpm.app")
    ...
    def test_password_validity(self):
        url = reverse("sign-in")
        response = self.client.post(
            url, {"email": "user@jetpm.app", "password": "user123"}, format="json"
        )
    ...
    def test_user_login(self):
        ...
        response = self.client.post(
            url,
            {"email": "user@jetpm.app", "password": "user@123"},
            format="json",
        )
...
class MagicLinkGenerateEndpointTests(BaseAPITest):
    ...
    def test_magic_generate(self):
        ...
        ri = redis_instance()
        ri.delete("magic_user@jetpm.app")

        response = self.client.post(url, {"email": "user@jetpm.app"}, format="json")
        self.assertEqual(response.status_code, status.HTTP_200_OK)
...
class MagicSignInEndpointTests(BaseAPITest):
    ...
    def test_expired_invalid_magic_link(self):
        ...
        response = self.client.post(
            url,
            {"key": "magic_user@jetpm.app", "token": "xxxx-xxxxx-xxxx"},
            format="json",
        )
        self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)
        self.assertEqual(
    ...
    def test_invalid_magic_code(self):
        ...
        ## Create Token
        url = reverse("magic-generate")
        self.client.post(url, {"email": "user@jetpm.app"}, format="json")
    ...
    def test_magic_code_sign_in(self):
        ...
        # Get the token
        user_data = json.loads(ri.get("magic_user@jetpm.app"))
        token = user_data["token"]

        url = reverse("magic-sign-in")
        response = self.client.post(
            url,
            {"key": "magic_user@jetpm.app", "token": token},
            format="json",
        )
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(
